    ...
    visitor: {
      Program: {
        enter: function (path, state) {
          ...
          var jsxPragma = null;
          if (path.node && path.node.innerComments) {
            path.node.innerComments.forEach(function(comment) {
              if (comment.type === 'CommentBlock') {
                var extracted = comment.value.match(/^\*\s+@jsx\s+([\S]+)\s*$/);
                if (extracted) {
                  jsxPragma = extracted[1];
                }
              }
            });
          }
