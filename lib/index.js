...
function getVNodeType(astNode) {
  ...
  if (astType === 'JSXIdentifier') {
    ...
    if (astNode.property.name === 'Fragment' && (astNode.object.name === 'Inferno' || astNode.object.name === 'React')) {
      vNodeType = TYPE_FRAGMENT;
    } else {
      vNodeType = TYPE_COMPONENT;
  ...
  for (var j = 0; j < vChildren.elements.length; j++) {
    ...
    if (aChild.type === 'StringLiteral') {
      vChildren.elements[j] = t.callExpression(
        t.identifier(fileState.get('pragma') || opts.pragmaTextVNode || 'createTextVNode'),
        [aChild]
      );
    }
  }
...
function createVNode(astNode, opts, fileState, defineAll) {
  ...
  case 'JSXFragment':
    ...
    return t.callExpression(
      t.identifier(fileState.get('pragma') || opts.pragmaFragmentVNode || 'createFragment'),
  ...
  case 'JSXElement':
    ...
    case TYPE_COMPONENT:
      ...
      createVNodeCall = t.callExpression(
        t.identifier(fileState.get('pragma') || opts.pragmaCreateComponentVNode || 'createComponentVNode'),
    ...
    case TYPE_ELEMENT:
      ...
      createVNodeCall = t.callExpression(
        t.identifier(fileState.get('pragma') || opts.pragma || 'createVNode'),
        createVNodeArgs(
          vProps.flagsOverride || flags,
          vType.type,
          vProps.className,
    ...
    if (vProps.needsNormalization) {
      ...
      createVNodeCall = t.callExpression(
        t.identifier(fileState.get('pragma') || opts.pragmaNormalizeProps || 'normalizeProps'),
        [createVNodeCall]
      );
    }
...
function visitorEnter(path, state) {
  ...
  var pragma = state.file.get('pragma');
  if (pragma == null && (opts.imports === false || opts.imports === 'false')) {
    ...
    visitor: {
      Program: {
        enter: function (path, state) {
          ...
          // Get the file JSX pragma
          var jsxPragma = null;
          var firstChild = path.get('body.0');
          if (firstChild && firstChild.node && firstChild.node.leadingComments) {
            firstChild.node.leadingComments.forEach(function(comment) {
              if (comment.type === 'CommentBlock') {
                var annotation = comment.value.match(/^\*\s+@jsx\s+([\S]+)\s*$/);
                if (annotation) {
                  jsxPragma = annotation[1];
                }
              }
            })
          }
          fileState.set('pragma', jsxPragma);
        },
        exit: function (path, state) {
          var fileState = state.file;

          var needsAnyImports = Boolean(
            fileState.get('pragma') == null &&
            (
